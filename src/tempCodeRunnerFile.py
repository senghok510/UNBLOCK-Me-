...
class RushHour:
        ...
        for vehicle in self.vehicles:
            ...
            if vehicle.orientation == 'v':
                ...
                if y_pos > 0 and self.grid[y_pos - 1][x_pos] == ".":
                    updated_vehicles = list(copy.deepcopy(self.vehicles))
                    ...
                    for i, v in enumerate(updated_vehicles):
                        if v.label == vehicle.label:
                            ...
                            break
                    possible_moves.add(tuple(updated_vehicles)) 
    ...
    def check_winning(self):
        """Checks if the red car has reached the exit (right edge of the grid)."""

        for vehicle in self.vehicles:
            if vehicle.label == 1:  # Ensure we correctly identify the red car
                if vehicle.orientation == "h":  
                    # Check if any part of the car reaches the rightmost column
                    if vehicle.x + vehicle.length - 1 == self.grid_size:
                        return True  
        return False  
