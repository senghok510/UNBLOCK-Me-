...
class RushHour:
    def __init__(self, grid_size, vehicles):
        ...
        self.grid = [[None] * grid_size for _ in range(grid_size)]  # Initialize grid with None
    ...
    def _place_vehicles_on_grid(self):
        ...
        for vehicle in self.vehicles:
            for x, y in vehicle.Position_vehicle():
                ...
                # Place the vehicle's label on the grid
                self.grid[y - 1][x - 1] = vehicle.label

    def display_grid(self):
        """Displays the current state of the Rush Hour game."""
        print("   " + " ".join(f"{i+1:>2}" for i in range(self.grid_size)))  # Column headers
        print("   " + "---" * self.grid_size)  # Top border
        for i, row in enumerate(self.grid):
            row_display = " ".join(str(cell) if cell is not None else '.' for cell in row)
            print(f"{i+1:>2} | {row_display}")  # Row label with grid contents
        print("   " + "---" * self.grid_size)  # Bottom border
